...
- [ ] Milestone 3 started
- [ ] Post GREEN design review completed
...
- `website/README.md`: update. It currently says generated pages project `SKILL.md` and archived `report.json` files and describes only skill level evidence. It must document suite, case, prompt, fixture, optional coverage sources, case evidence states, new routes, generated data additions, historical pages, linked flows, and Operations terminology.
- `website/content-config.json`: expected no change. The feature adds no public configuration or catalog override; the file continues to own the base path and disabled skills. Confirm this after implementation.
- Root `README.md`: update its public website description so it mentions declared evaluation cases and archived operation evidence rather than only archived evidence.
- `CODEX_CLI.md`: expected no change. The feature adds no CLI command, flag, installation behavior, runner behavior, or maintainer recipe. Confirm this after implementation.
- `EVALUATIONS.md`: expected no change. It already defines case anatomy, runner sequencing, fingerprints, results, durable evidence, and promotion. The website is a projection of that contract rather than a new evaluation contract. Confirm this after implementation.
- `website/.generated/`: never edit. It remains disposable output regenerated by website commands.
